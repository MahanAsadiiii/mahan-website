...
const inputElem = $.querySelector('#amount')
const displayArea = $.querySelector('#result')
let resultArea = $.querySelector('#display-div')
...
function generating(T){
  ...
  // this is for making random number and after that we multiply in our array for making random number depends our array
  let randomTextIndex = Math.floor(Math.random() * dummyText.length)
  

  if(InputValue < 0 || InputValue > 9 || InputValue === '' ||isNaN(InputValue)){
    displayArea.innerHTML = dummyText[randomTextIndex]
  }else{
    // we try to slice dummyarray for extarct paragraph
    let sliceArray =dummyText.slice( 0, InputValue)
    // this is for putting textes in the paragraph bcs its block elem and at the end we join it after eachother
    let finaltextArray = sliceArray.map(function(item){
      return '<p>'+ item + '</p>'
    }).join('')
    displayArea.innerHTML = finaltextArray
  }
  
}


submitBtn.addEventListener('click', generating )