...
const ModalObject = (prop: props) => {
    ...
    return (
        <div onClick={showModal} className=' text-center py-3 rounded-lg bg-white text-gray-700 cursor-pointer w-full'>
            <h6 >{prop.btnText}</h6>
            <Modal width={800} cancelText="بستن" title="پروژه‌ها" open={isModalOpen} onCancel={handleCancel} className='font-[estedad-bold]'>
                <div className="grid grid-cols-2 gap-x-10">
                    {prop.ModalContent.map((item, index) => {
                        return (
                            <div className='flex justify-between my-3 items-baseline' key={item.id}>
                                <div className="flex items-baseline gap-2">
                                    <h6 className='text-lg'>{index + 1}-</h6>
